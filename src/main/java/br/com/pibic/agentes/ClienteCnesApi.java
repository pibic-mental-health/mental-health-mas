...
public class ClienteCnesApi {
    ...
    public static List<LocalAtendimentoResultado> buscarLocais(
        ...
        List<String> codigosMunicipio =
                ClienteIbgeApi
                        .obterCodigosMunicipioPossiveis(
                                cidade,
                                uf
                        );

        if (codigosMunicipio.isEmpty()) {

            /*
             * Fallback para manter compatibilidade
             * caso a API do IBGE esteja indisponivel.
             */
            codigosMunicipio =
                    obterCodigosMunicipioFallback(
                            cidade,
                            uf
                    );
        }
        ...
        if (codigoUf.isEmpty()
                || codigosMunicipio.isEmpty()) {

            System.out.println(
                    "[CNES_API] Nao foi possivel resolver "
                    + "UF/municipio para "
                    + cidade
                    + "/"
                    + uf
        ...
        for (String codigoMunicipio : codigosMunicipio) {
            ...
            /*
             * Evita repetir a mesma consulta em formato
             * de 6 e 7 digitos quando a primeira funcionou.
             */
        ...
        System.out.println(
                "[CNES_API] Locais relevantes encontrados: "
                + locais.size()
        );
    ...
    private static String executarGet(
        ...
        try {
            ...
            if (statusCode == 401
                ...
                System.out.println(
                        "[CNES_API] API exigiu autenticacao. "
                        + "Configure SAUDE_API_TOKEN se necessario."
                );
    ...
    private static List<LocalAtendimentoResultado> extrairLocaisDaResposta(
        ...
        try {
            ...
            System.out.println(
                    "[CNES_API] Erro ao interpretar JSON: "
                    + e.getMessage()
            );
        }
    ...
    private static void coletarObjetos(
    ...
    private static boolean pareceEstabelecimento(
            JsonObject obj) {
    ...
    private static LocalAtendimentoResultado converterObjetoParaLocal(
        ...
        local.observacao =
                "Fonte institucional do Cadastro Nacional de "
                + "Estabelecimentos de Saude. Confirmar funcionamento "
                + "e contato antes do deslocamento.";

        return local;
    }

    private static String buscarValorPorChaves(
            JsonObject obj,
            String... chaves) {
    ...
    private static String buscarValorRecursivo(
            JsonElement elemento,
            String chaveBuscada) {
    ...
    private static String obterCodigoUf(
            String uf) {
    ...
    private static List<String> obterCodigosMunicipioFallback(
            String cidade,
            String uf) {
